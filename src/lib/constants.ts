...
export const MOTION_PROMPTS: Record<MotionStyle, string> = {
  ...
  wink: `${PROMPT_BASE} ACTION — WINK: The person gives a quick natural wink with one eye only, while the other eye remains open, with a soft smile. Only ONE eye closes briefly and reopens — the other eye MUST stay fully open the entire time. This is NOT a blink (both eyes closing) and NOT both eyes closing. Pair with a soft warm closed-mouth smile and a tiny head tilt. If multiple faces are present, only the primary (largest, most centered) face winks; others keep a calm, slightly smiling expression. ${MOUTH_CLOSED}`,
  clap: `${PROMPT_BASE} ACTION — CLAP: The person raises both hands in front of the chest and performs a small, realistic clap — palms meet smoothly 2–3 times across the 5 seconds. Hands stay anatomically correct (five fingers, no morphing), motion is gentle and contained, with a friendly closed-mouth smile. ${MOUTH_CLOSED}`,

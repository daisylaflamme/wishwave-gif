...
export const MOTION_PROMPTS: Record<MotionStyle, string> = {
  ...
  wink: `${PROMPT_BASE} ACTION — WINK: The person performs a subtle, natural single-eye wink — one eye gently closes and reopens once, paired with a soft warm closed-mouth smile and a tiny head tilt. Keep the other eye open and natural. If multiple faces are present, only the primary (largest, most centered) face winks; others keep a calm, slightly smiling expression. ${MOUTH_CLOSED}`,

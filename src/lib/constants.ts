...
export const MOTION_PROMPTS: Record<MotionStyle, string> = {
  ...
  wink: `${PROMPT_BASE} ACTION — WINK: The primary person gives a quick natural wink using only ONE eye while the other eye remains FULLY OPEN the entire time. Not a blink, not both eyes closing. Keep head position stable, add a subtle soft smile only, no exaggerated facial movement. Do NOT create new people, duplicate faces, or alter the background. If multiple faces are present, only the primary (largest, most centered) face winks; others keep a calm, slightly smiling expression. ${MOUTH_CLOSED}`,
